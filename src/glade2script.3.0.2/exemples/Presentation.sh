...
FindNewVersion()
{
new_version=$(sed -n '/name=glade2script/s/.*glade2script.\(3.*\).tar.gz.*/\1/p' ./version.html | head -n1)
}
...
vd=$(sed -n '\@debug("glade2script@s@.*debug("glade2script \([^,]*\),.*, \(.*\)".@\1, \2@p' ../glade2script.py)
vd=${vd/\'/}
version=${vd%%,*}
date=${vd#* }
