    ...
    def find_tree_item(self, modele, col, item):
        ...
        try:
            if re.search(item.replace('(','\(').replace(')','\)'), donnees[int(col)]):
                print donnees
                liste.append([path,iter, path.to_string()])
        except:
            #raise
            item = item.split('|')
            if item == donnees:
    ...
    def stop(self,arg=None):
        if import_py is not None:
            ...
            if arg == 'no':
                ...
                self.stop_save()
                print 'VERSION="glade2script 2.3.0, Copyright (C) 2010-2011, November 2011"'
                print 'EXIT="yes"'
                EXIT='yes'
        if term_box:
...
if __name__ == '__main__':
    ...
    try:                                
        opts, args = getopt.getopt(sys.argv[1:], "hs:g:r:t:d",
            ...
            "gtkrc=", "tree=","systray=","webcache=","webmenu=",
            ...
            "gtkbuilder","combobox=","transparent=","webloaded="]
                    )
    ...
    debug("glade2script 3.0.2, Copyright (C) 2010-2012, Janvier 2012")
    m = Gui()
    m.go_thread()
    m.main()
